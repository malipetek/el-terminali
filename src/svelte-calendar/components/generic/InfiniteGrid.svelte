<script>
	...
	const getCached = (index) => {
		let result; 
		let indexed = $visibleData.find(({ index: i }) => i === index);
			result = indexed;
			result = result ? result.data : get(index);
		return result;
	}
...
<div class="grid" style={gridStyle} bind:clientHeight={$dim.h} bind:clientWidth={$dim.w}>
	{#each $visibleData as obj (obj.data && obj.data[idKey] ? obj.data[idKey] : obj.index)}
